...
with pm.Model() as model_mcmc:
    ...
    mu = pm.Uniform("mu", lower=0, upper=1e-5,)
    # gamma_I = pm.Uniform(
    #     "gamma_I",
    #     lower=1 / 21,
    #     upper=1 / 14,
    ...
    d_D = pm.Uniform("d_P", lower=1e-5, upper=0.1,)
    ...
    E0_uncertain = pm.Uniform("E0", lower=P0, upper=10 * P0)
    A0_uncertain = pm.Uniform("A0", 1, upper=P0)
    I0_uncertain = pm.Uniform("I0", lower=P0, upper=5 * P0)
    standard_deviation = pm.Uniform("std_deviation", lower=1e1, upper=7e2)
    ...
    fitting_model = pm.Deterministic(
        ...
        seirpdq_ode_wrapper_with_y0(
            ...
            beta,
            mu,
            theano.shared(gamma_I_deterministic),
            theano.shared(gamma_A_deterministic),
            gamma_P,
            d_I,
            d_D,
            epsilon_I,
    ...
    # startsmc = {v.name: np.random.uniform(1e-3, 2, size=draws) for v in model_mcmc.free_RVs}
