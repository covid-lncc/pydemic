...
df_brazil_cases_by_day = df_brazil_cases_by_day[df_brazil_cases_by_day.day > 0]
...
S0 = target_population - (E0 + I0 + R0 + D0 + Q0)
y0_seirpdq = S0, E0, I0, R0, D0, Q0, C0, H0  # SEIRPDQ IC array (not fully used)
# print(y0_seirpdq)
...
@jit(nopython=True)
def smooth_transition(t, value_1, value_2, t_transition, smoothing_factor=1):
    delta_t = (t - t_transition) * smoothing_factor
    factor_1 = (1 - np.tanh(delta_t)) / 2
    factor_2 = (np.tanh(delta_t) + 1) / 2
    return factor_1 * value_1 + factor_2 * value_2
...
def seirpdq_model(
    ...
    # beta = beta0 if t < 10 else theta * beta0
    # eta = 0.25 if 120 <= t < 123 else eta
    # eta = smooth_activation(t, 0.05, 120, 130)
    # eta = smooth_transition(t, 0.0, 0.5 * omega, 0)
    # N = smooth_transition(t, 1.0, 1.5, 30)
    # omega = omega if 60 < t else 0.5 * omega
    S_prime = -beta / N * S * I - omega * S + eta * Q
    E_prime = beta / N * S * I - sigma * E  # - omega * E
    I_prime = sigma * E - gamma_I * I - d_I * I  # - omega * I
    R_prime = gamma_I * I  # - omega * R
    D_prime = d_I * I
    # Q_prime = omega * (S + E + I + R) - eta * Q
    Q_prime = omega * S - eta * Q
    C_prime = sigma * E
    H_prime = gamma_I * I
    return S_prime, E_prime, I_prime, R_prime, D_prime, Q_prime, C_prime, H_prime
...
def seirpdq_least_squares_error_ode(
    ...
    weighting_for_exp3_constraints = 0e0
    ...
    try:
        ...
        residual1 = f_exp1 - simulated_qoi1  # Deaths
        residual2 = f_exp2 - simulated_qoi2  # Cases
        residual3 = f_exp3 - simulated_qoi3  # Healed
        ...
        objective_function = 1 / num_of_qoi * (first_term + second_term + third_term)
    except ValueError:
...
result_seirpdq = optimize.differential_evolution(
    ...
    args=(
        data_time,
        [dead_individuals, confirmed_cases, recovered_cases],
        seirpdq_ode_solver,
        y0_seirpdq,
        target_population,
    ),
...
print(f"-- Confirmed cases estimate for today (SEIRPD-Q):\t{int(I_seirpdq.max())}")
print(
    f"-- Confirmed cases estimate population percentage for today (SEIRPD-Q):\t{100 * I_seirpdq.max() / target_population:.3f}%"
)
...
number_of_days_after_last_record = 150
...
crisis_day_seirpdq = np.argmax(I_predict_seirpdq)
...
# %%
print(
    f"-- Max number of diagnosed individuals (SEIRPD-Q model):\t {int(np.max(I_predict_seirpdq))}"
)
print(
    f"-- Population percentage of max number of diagnosed individuals (SEIRPD-Q model):\t {100 * np.max(I_predict_seirpdq) / target_population:.2f}%"
)
print(
