...
from scipy.stats import gaussian_kde  # to calculate stats summary
...
E0, A0, I0, P0, R0, D0, C0, H0, T0 = (
    int(30 * float(df_target_country.confirmed.values[0])),  # E0
    int(1 * float(df_target_country.confirmed.values[0])),  # A0
    int(8 * float(df_target_country.confirmed.values[0])),  # I0
    int(float(df_target_country.confirmed.values[0])),  # P0
    ...
    int(8 * float(df_target_country.confirmed.values[0])),  # T0
)

S0 = target_population - (E0 + A0 + I0 + R0 + P0 + D0)
y0_seirpdq = S0, E0, A0, I0, P0, R0, D0, C0, H0  # T0  # SEIRPDQ IC array (not fully used)
# print(y0_seirpdq)
...
# %%
@jit(nopython=True)
def omega_t(t, omega_value, decay_constant=0.0):
...
def seirpdq_model(
    ...
    I_prime = sigma * rho * E - gamma_I * I - d_I * I - omega * I - epsilon_I * theta * I
    P_prime = epsilon_I * theta * I - gamma_P * P - d_P * P
    R_prime = gamma_A * A + gamma_I * I + gamma_P * P + omega * (S + E + A + I) - eta * R
    D_prime = d_I * I + d_P * P
    C_prime = epsilon_I * theta * I
    H_prime = gamma_P * P
    # T_prime = sigma * rho * E
    return (
        S_prime,
...
def calculate_reproduction_number(
    S0, beta, mu, gamma_A, gamma_I, d_I, epsilon_I, theta, rho, omega, sigma=1 / 7
):
    left_term = sigma * (1 - rho) * mu / ((sigma + omega) * (gamma_A + omega))
    right_term = (
        beta * sigma * rho / ((sigma + omega) * (gamma_I + d_I + omega + theta * epsilon_I))
    )
...
parameters_dict = {
    "Model": "SEAIRPD-Q",
    "$beta$": beta_deterministic,
    "$mu$": beta_deterministic,
    "$gamma_I$": gamma_I_deterministic,
    "$gamma_A$": gamma_A_deterministic,
    "$gamma_P$": gamma_P_deterministic,
    "$d_I$": d_I_deterministic,
    "$d_P$": d_P_deterministic,
    "$epsilon_I$": epsilon_I_deterministic,
    "$rho$": rho_deterministic,
    "$omega$": omega_deterministic,
    "$sigma$": sigma_deterministic,
}
...
#     t_computed_seirpdq,
#     T_seirpdq,
#     label="Total symptomatic (SEAIRPD-Q)",
#     marker="D",
#     linestyle="-",
#     markersize=10,
# )
...
plt.plot(
    t_computed_predict_seirpdq, Rt_predict, "r", marker="X", linestyle="-", markersize=5,
)
...
def seirpdq_ode_wrapper_with_y0(
    ...
    # (_, _, _, _, _, _, simulated_qoi1, simulated_qoi2, _, _) = simulated_ode_solution
...
with pm.Model() as model_mcmc:
    ...
    d_I = pm.Uniform("d_I", lower=0, upper=1e-1)
    ...
    seirdpq_trace_calibration = pm.sample_smc(
        draws=draws, n_steps=25, parallel=True, cores=48, progressbar=True, random_seed=seed
    )
...
def _scalar_rv_mvp_estimation(rv_realization_values: np.ndarray) -> np.ndarray:
    ...
    equally_spaced_samples = np.linspace(
        rv_realization_values.min(), rv_realization_values.max(), num_of_realizations
    )
...
for variable in progress_bar:
    progress_bar.set_description("Arviz post-processing plots")
...
df_stats_summary = az.summary(
    data=seirdpq_trace_calibration, var_names=calibration_variable_names, kind="stats", round_to=15,
)
...
print("-- Post-processing arviz joint dists")
az.plot_pair(
...
for realization in trange(number_of_total_realizations):
    ...
    # S, E, A, I, P, R, D, C, H, T = y_computed_predict
...
T_predicted = np.array(H_predicted)
...
# T_min = np.percentile(T_predicted, percentile_cut, axis=0)
# T_max = np.percentile(T_predicted, 100 - percentile_cut, axis=0)
# T_mean = np.percentile(T_predicted, 50, axis=0)
...
#     T_mean,
#     "g",
#     label="Total symptomatics (SEAIRPD-Q)",
#     marker="X",
#     linestyle="-",
#     markersize=10,
# )
# plt.fill_between(t_computed_predict, T_min, T_max, color="g", alpha=0.2)
...
plt.plot(
    t_computed_predict, Rt_mean, "r", marker="X", linestyle="-", markersize=3,
)
