...
confirmed_plot = (
    ...
    .encode(
        ...
        y=alt.Y("active", axis=alt.Axis(title="Active cases")),
    )
)
...
def altair_plot_for_confirmed_and_deaths(
    ...
    confirmed_plot = (
        ...
        .encode(
            ...
            y=alt.Y("active", axis=alt.Axis(title="Cases"), title="Active"),
...
y0_seirpdq = S0, E0, A0, I0, P0, R0, D0  # SEIRPDQ IC array
print(y0_seirpdq)
...
# rho_deterministic,
# omega_deterministic,
# eta_deterministic) = result_seirpdq.x
...
# %%
print(f"Max number of diagnosed individuals (SEIRPD-Q model):\t {int(np.max(P_predict_seirpdq))}")
print(
...
def seirpdq_ode_wrapper(
    ...
    args = [beta, mu, gamma_I, gamma_A, gamma_P, d_I, d_D, epsilon_I, rho, omega]
...
with pm.Model() as model_mcmc:
    ...
    mu = pm.Uniform(
        "mu", lower=(1 - percent_uq) * mu_deterministic, upper=(1 + percent_uq) * mu_deterministic
    )
    gamma_I = pm.Uniform(
        "gamma_I",
    ...
    gamma_A = pm.Uniform(
        "gamma_A",
        lower=(1 - percent_uq) * gamma_A_deterministic,
        upper=(1 + percent_uq) * gamma_A_deterministic,
    )
    gamma_P = pm.Uniform(
        "gamma_P",
        lower=(1 - percent_uq) * gamma_P_deterministic,
        upper=(1 + percent_uq) * gamma_P_deterministic,
    )
    d_I = pm.Uniform(
        "d_I",
        lower=(1 - percent_uq) * d_I_deterministic,
        upper=(1 + percent_uq) * d_I_deterministic,
    )
    d_D = pm.Uniform(
        "d_D",
        lower=(1 - percent_uq) * d_D_deterministic,
        upper=(1 + percent_uq) * d_D_deterministic,
    )
    epsilon_I = pm.Uniform(
        "epsilon_I",
        lower=(1 - percent_uq) * epsilon_I_deterministic,
        upper=(1 + percent_uq) * epsilon_I_deterministic,
    )
    rho = pm.Uniform(
        "rho",
        lower=(1 - percent_uq) * rho_deterministic,
        upper=(1 + percent_uq) * rho_deterministic,
    )
    omega = pm.Uniform(
        "omega",
        lower=(1 - percent_uq) * omega_deterministic,
        upper=(1 + percent_uq) * omega_deterministic,
    )
    # eta = pm.Uniform('eta', lower=(1 - percent_uq) * eta_deterministic, upper=(1 + percent_uq) * eta_deterministic)
    E0_uncertain = pm.Uniform("E0", lower=(1 - percent_uq) * E0, upper=(1 + percent_uq) * E0)
    A0_uncertain = pm.Uniform("A0", lower=(1 - percent_uq) * A0, upper=(1 + percent_uq) * A0)
    I0_uncertain = pm.Uniform("I0", lower=(1 - percent_uq) * I0, upper=(1 + percent_uq) * I0)
    ...
    seirdpq_trace = pm.sample(5000, chains=4, cores=4, step=step, random_seed=seed)


# %%
percentile_cut = 5

y_min = np.percentile(seirdpq_trace["seirpdq_model"], percentile_cut, axis=0)
y_max = np.percentile(seirdpq_trace["seirpdq_model"], 100 - percentile_cut, axis=0)
y_fit = np.percentile(seirdpq_trace["seirpdq_model"], 50, axis=0)

# %%
has_to_plot_infection_peak = True

crisis_day_fit = np.argmax(y_fit[4])
crisis_day_min = np.argmax(y_min[4])
crisis_day_max = np.argmax(y_max[4])
...
if has_to_plot_infection_peak:
    plt.axvline(
        x=crisis_day_fit, color="blue", linestyle="-", label="Diagnosed peak (SEAIRPD-Q)", alpha=0.5
    )
    plt.axvline(x=crisis_day_min, color="blue", linestyle="--", alpha=0.5)
    plt.axvline(x=crisis_day_max, color="blue", linestyle="--", alpha=0.5)

plt.plot(
    time_range, y_fit[-1], "r", label="Deaths (SEAIRPD-Q)", marker="v", linestyle="-", markersize=10
)
plt.fill_between(time_range, y_min[-1], y_max[-1], color="r", alpha=0.2)
...
# %%
# population_uncertain_variance = 0.5 * np.max(dead_individuals) * target_population
population_uncertain_variance = 100
...
with pm.Model() as model_mcmc:
    ...
    rho = pm.Uniform(
        "rho",
        lower=(1 - percent_calibration) * rho_deterministic,
        upper=(1 + percent_calibration) * rho_deterministic,
    )
    omega = pm.Uniform(
        "omega",
    ...
    # std_deviation = pm.Uniform('std_deviation', lower=1e1, upper=1e4)
    ...
    seirdpq_trace_calibration = pm.sample(
        65000, chains=8, cores=8, step=step, random_seed=seed, tune=20000
    )
...
pm.traceplot(seirdpq_trace_calibration[::plot_step], var_names=("d_D"))
...
pm.plot_posterior(
    seirdpq_trace_calibration[::plot_step], var_names=("d_D"), kind="hist", round_to=5
)
...
d_D_prediction = seirdpq_trace_calibration.get_values("d_D")
...
for realization in trange(number_of_total_realizations):
    ...
    S, E, A, I, P, R, D = y_computed_predict
