...
def seiaprdq_get_qoi_ic(x):
    diagnosed = np.sum( x[:, 4] )
    deaths = x[-1, -1]
    

    return np.sqrt(diagnosed**2 + deaths **2)
...
for i in range(0, len(endtimes)):
    ...
    for j, param in enumerate(seiaprdq_param_values):
        ...
        seiaprdq_qoi_values[i, j] = seiaprdq_get_qoi_ic( x )
...
ax.legend()
ax.set_ylim(0, 1.15)# Model Results (samples)
