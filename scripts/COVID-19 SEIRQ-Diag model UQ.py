...
# bounds_seirdaq = num_of_parameters_to_fit_seirdaq * [(0, 0.5)]
...
# %%
has_to_run_forward_uq = False
has_to_run_calibration = True
...
# print(f"parameters: gamma_A = {gamma_A_deterministic}, gamma_D = {gamma_D_deterministic}")
# print(f"parameters: d_I = {d_I_deterministic}, d_D = {d_D_deterministic}")
...
if has_to_run_forward_uq:
    ...
    with pm.Model() as model_mcmc:
        ...
        # seirdaq_trace = pm.sample(5000, chains=number_of_cores, cores=number_of_cores, step=step, random_seed=seed)
        seirdaq_trace = pm.sample(5000, chains=number_of_cores, step=step, random_seed=seed)
        # seirdaq_trace = pm.sample(4000, cores=number_of_cores,  step=step, random_seed=seed)
        # seirdaq_trace = pm.sample(4000, step=step, random_seed=seed)
...
# plt.savefig('seirdaq_d_I_traceplot_cal.png')
# plt.show()

# pm.traceplot(seirdaq_trace_calibration[::plot_step], var_names=('d_D'))
# plt.savefig('seirdaq_d_D_traceplot_cal.png')
# plt.show()
